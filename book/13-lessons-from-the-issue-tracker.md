# 13. Lessons from the Issue Tracker
...
It is a survey of the open, genuinely cache-related issues on the Umbraco CMS tracker, read in full (body plus comments) and grouped into the patterns that matter for Umbraco 17 and 18.[^14-survey]
...
This layer did not exist in the NuCache era. It is keyed by document GUID, it is refreshed off `ContentTreeChangeNotification` (which fires on **save**, not only on publish), and it has weak draft-versus-published guards in places. That combination explains a whole family of "my page suddenly 404s" reports.
...
```mermaid
flowchart TD
    A["Cache-related issues, v17/v18"] --> B["URL / routing cache<br/>DocumentUrlService"]
    A --> C["Cold start, upgrade,<br/>migration"]
    A --> D["Distributed invalidation<br/>and load balancing"]
    A --> E["HybridCache internals<br/>locking, traversal, config"]
    A --> F["Historical NuCache .db<br/>why HybridCache exists"]

    B --> B1["Pages lose their links / 404"]
    B --> B2["Alias rebuilt from draft state"]
    B --> B3["Key change blocked by foreign key"]
    B --> B4["Bulk save crashes on SqlBulkCopy"]

    C --> C1["404 after migrations"]
    C --> C2["Content blank after 13->17 upgrade"]
    C --> C3["Cold boot rebuilds everything"]

    D --> D1["Publisher stops broadcasting"]
    D --> D2["Transient Delivery API 404s"]
    D --> D3["Publish notifications race the cache"]
    D --> D4["Index rebuild not propagated"]

    E --> E1["Read-lock timeouts"]
    E --> E2["Traversal warms whole cache"]
    E --> E3["Member cache cannot be disabled"]
```
...
These touch the published cache engine itself.
...
Read together, these explain the move to `HybridCache`: local `.db` files that lock during upgrades and balloon memory at start-up are exactly the fragilities a database-backed, `IMemoryCache`/`IDistributedCache`-layered cache was designed to remove. See [chapter 12](./11-nucache-vs-hybrid-cache.md).
...
- [Chapter 11 - NuCache vs Hybrid Cache](./11-nucache-vs-hybrid-cache.md) for historical context on why the architecture changed.

## Sources

- Issue tracker survey captured 30 June 2026; see [F4 in the appendix](./10-appendix-sources.md#f4-open-cache-issue-survey-v17v18) for the full issue list and [F5](./10-appendix-sources.md#f5-cache-related-fixes-and-pull-requests) for the referenced pull requests.
- Related chapters: [04 - Cache Busting and Invalidation](./04-cache-busting-and-invalidation.md), [09 - Future Hybrid Cache Architecture](./09-future-hybrid-cache-architecture.md), [11 - NuCache vs Hybrid Cache](./11-nucache-vs-hybrid-cache.md), [12 - Examine, Indexes, and Cache-Adjacent Querying](./12-examine-indexes-and-cache-adjacent-querying.md).

[^14-survey]: See [F4 in the appendix](./10-appendix-sources.md#f4-open-cache-issue-survey-v17v18). All issues were read in full, including comments, on 30 June 2026.
[^14-fixes]: See [F5 in the appendix](./10-appendix-sources.md#f5-cache-related-fixes-and-pull-requests). Pull-request merge states were verified against the GitHub API on 30 June 2026.
