# 06. Published Content Cache, AppCaches, and Load Balancing
...
Sometimes it means "use an index instead of traversal", which is exactly where Examine fits. See [14 - Examine, Indexes, and Cache-Adjacent Querying](./14-examine-indexes-and-cache-adjacent-querying.md) for the full comparison.
...
For the fuller future direction, see [07 - How the Hybrid Cache Engine Works](./07-hybrid-cache-engine.md).
...
> **Gotcha — traversal is not free any more.** Code like `.Children().Where(...)` deserves more suspicion than it did in the old "keep everything hot" world: walking the tree can now hydrate and cache every node you touch. When the hard part is *finding* items, prefer an index (see [Chapter 14](./14-examine-indexes-and-cache-adjacent-querying.md)); when it is *recomputing* a small result, prefer `RuntimeCache`.
...
- [Chapter 9 - Cache Busting and Invalidation](./09-cache-busting-and-invalidation.md) — the invalidation choreography, in full.
- [Chapter 14 - Examine, Indexes, and Cache-Adjacent Querying](./14-examine-indexes-and-cache-adjacent-querying.md) — when *not* to solve a problem with a cache.
- [Chapter 7 - How the Hybrid Cache Engine Works](./07-hybrid-cache-engine.md) — where this architecture is heading.
...
[^03-l0]: See [C1](./17-appendix-sources.md#c1-umbraco-17-source-checkout) and [C4](./17-appendix-sources.md#c4-umbracopublishedcachehybridcache-on-main) in the appendix.
[^03-hybrid]: See [M2](./17-appendix-sources.md#m2-aspnet-core-hybridcache) and [C1](./17-appendix-sources.md#c1-umbraco-17-source-checkout) in the appendix.
[^03-db]: See [C4](./17-appendix-sources.md#c4-umbracopublishedcachehybridcache-on-main) and [C5](./17-appendix-sources.md#c5-claudemd-for-umbracopublishedcachehybridcache) in the appendix.
[^03-payload]: See [M6](./17-appendix-sources.md#m6-hybridcacheoptions), [U4](./17-appendix-sources.md#u4-cache-settings-for-umbraco-17), and [T1](./17-appendix-sources.md#t1-releasing-hybridcache-into-the-wild-with-umbraco) in the appendix.
