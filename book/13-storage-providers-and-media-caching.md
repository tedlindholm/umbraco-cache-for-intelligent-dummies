# 13. Storage Providers and Media Caching
...
- [Edge Cache in Front of the CDA](./05-edge-cache-in-front-of-the-cda.md) — the sibling CDN story for CDA JSON responses rather than media files; do not conflate the two "CDN" chapters.
- [Chapter 7: How the Hybrid Cache Engine Works](./07-hybrid-cache-engine.md) — more on the `HybridCache` model the package-source metadata-cache notes describe.
- [Chapter 8: NuCache vs Hybrid Cache](./08-nucache-vs-hybrid-cache.md) — how the published-content side compares.
- [Chapter 9: Cache Busting and Invalidation](./09-cache-busting-and-invalidation.md) — the refresher pipeline the media side deliberately sidesteps.
- [Chapter 18: UMB.FYI Archive Notes](./18-appendix-umbfyi-archive-notes.md) — media/CDN, Delivery API, and headless cache-adjacent material found in the archive.
...
[^09-three]: See [U14 in the appendix](./17-appendix-sources.md#u14-storage-providers-docs) and [S1](./17-appendix-sources.md#s1-umbracostorageproviders-repository).
[^09-hybrid]: See [S1](./17-appendix-sources.md#s1-umbracostorageproviders-repository) and [M2](./17-appendix-sources.md#m2-aspnet-core-hybridcache).
[^09-bust]: See [S1](./17-appendix-sources.md#s1-umbracostorageproviders-repository).
[^09-24days-media]: See [F9 in the appendix](./17-appendix-sources.md#f9-24days-caching-field-notes), especially the 2015 CDN article and the 2022 static-asset TagHelper article.
